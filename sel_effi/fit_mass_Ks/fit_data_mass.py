...
ch=TChain()
ch.Add("/scratch43/ssellam/calibration/Data/original_data/Ks_pipi/*_"+beam+"_*.root/KsTuple/DecayTree")
assert ch.GetEntries()!=0
df=RDataFrame(ch)
df=df.Define("pi_pl_ETA","-log(tan(asin(pi_pl_PT/pi_pl_P)/2))")
df=df.Filter("KS0_D2_PT>130&&KS0_D1_PT>130")
df=df.Filter("pi_mi_PIDK<0")
...
for i in range(len(eta_bins)-1):
    ...
    for j in range(len(pt_bins)-1):
        ...
        mass = ROOT.RooRealVar( "KS0_MM","",475,520) 
        
        _mass_branch= df_sel.AsNumpy(["KS0_MM"])
        arr_mass_branch= _mass_branch["KS0_MM"]
        ...
        ptext.Draw()
        can.SaveAs("Plots/can_data_before_"+str(eta_bins[i])+"_"+str(eta_bins[i+1])+"_"+str(pt_bins[j])+"_"+str(pt_bins[j+1])+"_"+mode+".pdf")
...
for i in range(len(eta_bins)-1):
    for j in range(len(pt_bins)-1):
        ...
        df_sel=df.Filter("pi_pl_PT>"+str(pt_bins[j])+"&&pi_pl_PT<"+str(pt_bins[j+1])+"&&pi_pl_ETA>"+str(eta_bins[i])+"&&pi_pl_ETA<"+str(eta_bins[i+1]))
        sel_cuts="pi_pl_TRACK_Type==3&&pi_pl_P>7000&&BCType==3&&pi_pl_TRACK_GhostProb<"+str(cuts["GhostP"][beam])+"&&pi_pl_IPCHI2_ORIVX<"+str(cuts["pseudoIP"][beam])
        ...
        ptext.SetFillStyle(0)
        ptext.Draw()
        can.SaveAs("Plots/can_data_after_"+str(eta_bins[i])+"_"+str(eta_bins[i+1])+"_"+str(pt_bins[j])+"_"+str(pt_bins[j+1])+"_"+mode+".pdf")
...
output_file_path = "sel_effi_data_"+beam+"_"+mode+".json"
