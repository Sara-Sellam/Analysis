...
ch=TChain()
#ch.Add("/scratch43/ssellam/calibration/Data/cleaned_data/Phi_kk/*_"+beam+"_*.root/DecayTree")
ch.Add("/scratch43/ssellam/calibration/Data/original_data/phi_kk/*_pA_*.root/PhiTuple/DecayTree")

assert ch.GetEntries()!=0
df=RDataFrame(ch)
df=df.Define("Kmi_ETA","-log(tan(asin(Kmi_PT/Kpl_P)/2))")
...
#"pt":array('d',[400,570, 740, 961, 1249, 1622, 2107, 2737, 3556, 4619]
...
#sel_cuts="Kpl_TRACK_Type==3&&Kpl_P>2000&&BCType==3&&nPVs==1&&Kpl_TRACK_GhostProb<"+str(cuts["GhostP"][beam])#+"&&Kpl_IPCHI2_ORIVX<"+str(cuts["pseudoIP"][beam])
...
for i in range(len(eta_bins)-1):
    for j in range(len(pt_bins)-1):
        ...
        if mode =="sys":
            ...
            resol = ROOT.RooRealVar('resol','resol',3,1, 5)
        ...
        ptext.Draw()
        can.SaveAs("/home3/sara.sellam/RpPb_identified_hadrons_project/efficiencies/sel_effi/fit_mass/Plots/can_data_"+beam+"_before_"+str(eta_bins[i])+"_"+str(eta_bins[i+1])+"_"+str(pt_bins[j])+"_"+str(pt_bins[j+1])+"_"+mode+"_"+selection+".pdf")
...
for i in range(len(eta_bins)-1):
    for j in range(len(pt_bins)-1):
        ...
        ptext.SetFillStyle(0)
        ptext.Draw()
        can.SaveAs("/home3/sara.sellam/RpPb_identified_hadrons_project/efficiencies/sel_effi/fit_mass/Plots/can_data_"+beam+"_after_"+str(eta_bins[i])+"_"+str(eta_bins[i+1])+"_"+str(pt_bins[j])+"_"+str(pt_bins[j+1])+"_"+mode+"_"+selection+".pdf")
        print("Kmi_PT>"+str(pt_bins[j])+"&&Kmi_PT<"+str(pt_bins[j+1])+"&&Kmi_ETA>"+str(eta_bins[i])+"&&Kmi_ETA<"+str(eta_bins[i+1])+"&&"+sel_cuts)
...
for i in range(len(eta_bins)-1):
    for j in range(len(pt_bins)-1):
        sig_dict[str(eta_bins[i])+"_"+str(eta_bins[i+1])][str(pt_bins[j])+"_"+str(pt_bins[j+1])]["sel_effi_data"]={}
        ...
        sig_dict[str(eta_bins[i])+"_"+str(eta_bins[i+1])][str(pt_bins[j])+"_"+str(pt_bins[j+1])]["sel_effi_data"]["value"]=ratio.nominal_value
        sig_dict[str(eta_bins[i])+"_"+str(eta_bins[i+1])][str(pt_bins[j])+"_"+str(pt_bins[j+1])]["sel_effi_data"]["error"]=ratio.std_dev

# Define the output file path
output_file_path = "/home3/sara.sellam/RpPb_identified_hadrons_project/efficiencies/sel_effi/sel_effi_data_"+beam+"_"+mode+"_"+selection+".json"
